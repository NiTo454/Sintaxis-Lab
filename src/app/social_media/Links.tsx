...
export default function Links() {
  ...
  return (
    <section id="enlaces" className="py-16 md:py-24 px-4 sm:px-6 max-w-4xl mx-auto flex flex-col items-center">
      ...
      >
        <ScrambleText text="03. REDES_Y_ENLACES" />
      </motion.h2>
      <motion.p
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
      ...
      <div className="w-full max-w-3xl grid grid-cols-1 sm:grid-cols-2 gap-4">
        {links.map((link, i) => {
          const isFullWidth = link.id === 'website';
          return (
            <motion.a
              key={link.id}
              href={link.url}
              ...
              whileHover={{ 
                ...
                boxShadow: `0 0 20px rgba(${link.rgbColor}, 0.15)`
              }}
            ...
            >
              ...
              <div 
                ...
                style={{ backgroundColor: `rgba(${link.rgbColor}, 0.08)` }}
              />
              ...
              <div className="flex items-center gap-4 z-10">
                ...
                <div className="flex flex-col text-left">
                  <span className="font-mono font-bold text-xs sm:text-sm text-zinc-200 group-hover:text-white uppercase tracking-wider transition-colors duration-300">
                    {link.name}
                  </span>
                  {link.description && (
                    <span className="text-[10px] text-zinc-500 font-mono mt-0.5 group-hover:text-zinc-400 transition-colors duration-300">
                      // {link.description}
                    </span>
                  )}
                </div>
              ...
              <ChevronRight size={18} className="text-zinc-600 group-hover:text-white group-hover:translate-x-1 transition-all duration-300 z-10 animate-pulse" />
            </motion.a>
          );
        })}
      </div>
